...
export class DrawingBoardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
     ...
     * Fabric Init
     */
    this.canvas = new fabric.Canvas('drawingBoard', {
      width: window.innerWidth * this.canvasScreenWidthPercentage,
      height: window.innerHeight * this.canvasScreenHeightPercentage,
    ...
    if (this.canvas) {
      ...
      if (this.roomId) this.listenToObjectEvents();

      if (this.roomId) this.listenToPeerChanges();
    }
  }
  ...
  toolsList = [
    ...
    {
      fontIcon: 'upload',
      toolTipText: 'Upload Background Image',
      toolName: 'upload',
      visibility: false,
      nextSpacer: false,
    },
    ...
    {
      fontIcon: 'info',
      toolTipText: 'Press Alt Key+ Drag With Select Tool to move canvas around',
      toolName: 'info',
      visibility: true,
      nextSpacer: false,
    },
  ...
  pushAllCurrentObjects() {
    this.canvas.getObjects().forEach((obj) => {
      let pushRef = push(
        ref(this.realtimeDb, `Whiteboard/${this.roomId}/Added`)
      );
  ...
  listenToPeerChanges() {
    ...
    this.objectAdditionUnsubscribe = onChildAdded(
      ref(this.realtimeDb, `Whiteboard/${this.roomId}/Added`),
      (snapshot) => {
    ...
    this.objectModifiedUnsubscribe = onChildChanged(
      ref(this.realtimeDb, `Whiteboard/${this.roomId}/Added`),
      (snapshot) => {
        if (snapshot.val().userId == this.userId) return;
        console.log(snapshot.val());
    ...
    this.objectRemovedUnsubscribe = onChildAdded(
      ref(this.realtimeDb, `Whiteboard/${this.roomId}/Removed`),
      (snapshot) => {
        if (snapshot.val().userId == this.userId) return;

        this.canvas.remove(
          this.canvas.getObjects().find((obj: any) => {
  ...
  listenToObjectEvents() {
      ...
      if (false) {
        object._objects.forEach((targetObject: any) => {
          ...
          set(
            ref(
              this.realtimeDb,
              `Whiteboard/${this.roomId}/Added/${targetObject.key}`
            ),
            json
          );
        });
        ...
        set(
          ...
          targetObject.toJSON(this.requiredKeys)
        );
      }
    });
    ...
    this.canvas.on('object:removed', (event: fabric.IEvent<MouseEvent>) => {
      ...
      set(
        push(ref(this.realtimeDb, `Whiteboard/${this.roomId}/Removed`)),
        targetObject.toJSON(this.requiredKeys)
      );
  ...
  unsubscribeSubscriptions() {
    if (this.objectAdditionUnsubscribe) this.objectAdditionUnsubscribe();

    if (this.objectModifiedUnsubscribe) this.objectModifiedUnsubscribe();

    if (this.objectRemovedUnsubscribe) this.objectRemovedUnsubscribe();
  }
  ...
  canvasMouseDownHandler(event: fabric.IEvent<MouseEvent>) {
    ...
    if (this.activeTool == this.toolsList[2].toolName) {
      ...
      if (!event.pointer?.x) {
        console.error(
          'Mouse Down Event Error in Rectangle Tool in: canvasMouseDownHandler'
        );
      }
      ...
      if (!event.pointer?.x) {
        console.error(
          'Mouse Down Event Error in Circle Tool in: canvasMouseDownHandler'
        );
      }
    }
    this.isMouseDown = true;
    ...
    if (this.isMouseDown) {
      if (this.activeTool == this.toolsList[5].toolName) {
        ...
        if (this.rectangleRef) {
          ...
          this.rectangleRef.set(
            'width',
            Math.abs(pointer.x - this.shapeMouseDownStartX)
          );
          this.rectangleRef.set(
            'height',
            Math.abs(pointer.y - this.shapeMouseDownStartY)
          );
        ...
        if (this.circleRef) {
          ...
          this.circleRef.set(
            'radius',
            Math.abs(pointer.x - this.shapeMouseDownStartX) / 2
          );
  ...
  selectTool(toolName: string) {
    ...
    if (toolName == this.toolsList[0].toolName) {
      ...
      this.changeRangeLimits(
        this.pencilRangeMin,
        this.pencilRangeMax,
        this.pencilRangeValue
      );
      ...
      this.selectedColor = this.selectedFillColor;
      this.changeRangeLimits(
        this.shapeRangeMinValue,
        this.shapeRangeMaxValue,
        this.shapeRangeValue
      );
      this.canvas.discardActiveObject();
  ...
  selectColor(color: string) {
    ...
    if (this.activeTool === this.toolsList[3].toolName) {
      ...
      for (let activeObject of this.canvas.getActiveObjects()) {
        ...
        if (flag) this.canvas.fire('object:modified', { target: activeObject });
      }
      this.canvas.requestRenderAll();
    }
    ...
    if (
      this.activeTool === this.toolsList[5].toolName ||
      this.activeTool === this.toolsList[6].toolName
    ) {
      /**
    ...
    if (uploadedFile.type.toString().toLowerCase().includes('image')) {
      ...
      reader.onload = (event) => {
        ...
        fabric.Image.fromURL(data, (img) => {
          ...
          this.canvas
            .setBackgroundImage(img, this.canvas.renderAll.bind(this.canvas), {
